...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  const num = parseInt(v.slice(0, 6) || '000000', 16);
  return [(num >> 16) & 255, (num >> 8) & 255, num & 255];
};

export interface CursorGridProps extends React.HTMLAttributes<HTMLDivElement> {
  /** If true, position fixed inset-0 across the entire page with window-level events */
  global?: boolean;
  cellSize?: number;
  color?: string;
  radius?: number;
...
const CursorGrid: React.FC<CursorGridProps> = ({
  ...
  useEffect(() => {
    ...
    const rebuild = () => {
      ...
      w = p.global ? window.innerWidth : (container.offsetWidth || window.innerWidth);
      h = p.global ? window.innerHeight : (container.offsetHeight || window.innerHeight);
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        ctx.beginPath();
        if (p.cellRadius > 0 && typeof (ctx as any).roundRect === 'function') {
          (ctx as any).roundRect(x, y, s, s, p.cellRadius);
        } else {
          ctx.rect(x, y, s, s);
        }
        if (p.fillOpacity > 0) {
    ...
    // Listeners for Global Mode (Window-level)
    ...
    // Listeners for Local Mode (Container-scoped)
    ...
    const onWindowFallbackPointerMove = (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect();
      if (
        e.clientX >= rect.left &&
    ...
    const onWindowFallbackPointerDown = (e: PointerEvent) => {
      if (!propsRef.current.clickPulse) return;
      const rect = canvas.getBoundingClientRect();
      if (
        e.clientX >= rect.left &&
        ...
        pulses.push({ x: e.clientX - rect.left, y: e.clientY - rect.top, t0: performance.now() });
        wake();
      }
    };
    ...
    if (global) {
      ...
      container.addEventListener('pointermove', onLocalPointerMove);
      container.addEventListener('pointerdown', onLocalPointerDown);
      window.addEventListener('pointermove', onWindowFallbackPointerMove, { passive: true });
      window.addEventListener('pointerdown', onWindowFallbackPointerDown);
    }
    ...
    return () => {
      ...
      if (global) {
        ...
        container.removeEventListener('pointermove', onLocalPointerMove);
        container.removeEventListener('pointerdown', onLocalPointerDown);
        window.removeEventListener('pointermove', onWindowFallbackPointerMove);
        window.removeEventListener('pointerdown', onWindowFallbackPointerDown);
      }
    };
  ...
  const containerClasses = [
    'cursor-grid',
    global ? 'cursor-grid--global fixed inset-0 pointer-events-none' : '',
    className
  ]
    .filter(Boolean)
    .join(' ');
